...
float readTempPel(uint8_t number) {
  ...
  if (number >= 0 and number <= 3) { //T1-T4
    ...
    R_NTC = r1 / ((32768 / vout) - 1); //calculating the resistance of the thermistor
    Temp_K = (R_NTC / TH1); //Temperature in Kelvin B_param and T0 = 25C TO K
    Temp_K = log(Temp_K);
    Temp_K /= bcoe1;
    Temp_K += 1.0 / (TH1 + 273.15);
    Temp_K = 1.0 / Temp_K;
...
void softStartPeltier() {
  for (uint16_t dutyCycle = 0; dutyCycle <= 255; dutyCycle++) {
